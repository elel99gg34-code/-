...
async function runSelfTest() {
  ...
  const script = `(async () => {
    ...
    try {
      ...
      check('rack has 390 instruments', mod.INSTRUMENTS.length === 390, mod.INSTRUMENTS.length);
      ...
      check('all 390 instruments trigger', bad.length === 0, bad.slice(0, 5).join(' | '));
      ...
      check('rack cards rendered', document.querySelectorAll('.icard').length === 390, document.querySelectorAll('.icard').length);
