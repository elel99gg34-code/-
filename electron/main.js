...
async function runSelfTest() {
  ...
  const script = `(async () => {
    ...
    try {
      ...
      check('rack has 890 instruments', mod.INSTRUMENTS.length === 890, mod.INSTRUMENTS.length);
      ...
      check('all 890 instruments trigger', bad.length === 0, bad.slice(0, 5).join(' | '));
      ...
      check('rack cards rendered', document.querySelectorAll('.icard').length === 890, document.querySelectorAll('.icard').length);
